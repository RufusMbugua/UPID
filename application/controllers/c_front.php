...
class C_Front extends CI_Controller {
	...
	public function report_listing() {
		...
		$query = $this -> db -> query($sql);
		$results = $query -> result_array();
		$dyn_table .= "<table class='table table-bordered'><thead style='background:#DDD;'><tr><th>Report Name</th><th>Modified</th><th>Options</th></tr></thead><tbody>";
		if ($results) {
			foreach ($results as $result) {
				$dyn_table .= "<tr><td><a href='".base_url()."reports/".$result['reportUrl']."'>" . strtoupper($result['reportTitle']) . "</a></td><td>" . date('d-M-Y',strtotime($result['dateModified'])) . "</td><td><a href='".base_url()."c_reports/getReport/".$result['reportID']."'>Details</a></td></tr>";
			}
		} else {
			$dyn_table .= "";
		}
